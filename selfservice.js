...
function renderMine() {
  ...
  el.innerHTML = '<div class="selfservice-card"><h2>Mijn bestellingen</h2><div class="selfservice-small selfservice-muted">Alleen bestellingen van dit apparaat worden hier getoond.</div></div>';
  entries.forEach(([id,o]) => {
    const st = statusInfo(o);
    const before = positionBefore(id,o);
    const card = document.createElement('div');
    card.className = 'selfservice-card';
    card.innerHTML = `
